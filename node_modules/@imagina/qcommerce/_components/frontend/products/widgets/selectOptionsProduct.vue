    ...
    <div v-for="(option, index) in optionsList" v-if="optionsList.length" :key="index">
      <div class="content-option" v-if="option.productOptionValues &&  option.productOptionValues.length">
        ...
        <div v-if="option.type == 'checkbox'">
          {{section[option.id].multiOption.length > 7 ? 'Máximo 7 opciones, en caso de selecionar mas, las ultimas no se tendran en cuenta en la orden.' : ''}}
          <q-select
            @input="setOptions(option.id, section[option.id].multiOption)"
            multiple
            chips
            v-model="section[option.id].multiOption"
            :options="setOptionForQselect(option.productOptionValues)"/>
  ...
  export default {
    ...
    methods: {
      ...
      setOptions(optionId, valuesId) {
        if (valuesId.length > 7){
          this.$helper.alert.error('Solo puede seleccionar Maximo 7 Opciones','bottom')
          valuesId.pop()
          return
        }
        //Reset local values
      ...
      setOptionForQselect(options){
        return options.map(option => {
          return {value: option.optionValueId , label:option.optionValue}
        })
      }
    },
  }
